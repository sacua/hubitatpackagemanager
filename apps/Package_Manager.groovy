	...
	return dynamicPage(name: "prefInstallChoices", title: "", nextPage: "prefInstallVerify", install: false, uninstall: false) {
		...
		if(pkgInstall) {
			...
			if (manifest.minimumHEVersion != null && !verifyHEVersion(manifest.minimumHEVersion)) {
				...
				def bundles = getOptionalBundlesFromManifest(manifest)
				def title = "Choose the components to install"
				if (apps.size() == 0 && drivers.size() == 0)
				title = "Ready to install"

				section("${title}") {
					if (apps.size() > 0 || drivers.size() > 0 || bundles.size() > 0)
...
def performInstallation() {
	...
	for (driverToInstall in driversToInstall) {					// required = false (aka optional)
...
def prefPkgModifyChoices() {
	...
	if (optionalLibraries?.size() > 0 || optionalApps?.size() > 0 || optionalDrivers?.size() > 0 || optionalBundles > 0) {
		...
		for (optBundle in optionalBundles) {
			if (isBundleInstalled(manifest, optBundle.key)) {
				installedOptionalBundles << optBundle.key
			}
		}
...
def prefVerifyPackageChanges() {
	...
	for (optBundle in bundlesToModify) {
		if (!isBundleInstalled(manifest,optBundle)) {
			...
			bundlesToInstallForModify << optBundle
			hasChanges = true
		}
	}
	bundlesToInstallStr += "</ul>"
...
def prefPkgRepairExecute() {
	...
	atomicState.refreshIntervalValue = atomicState.refreshIntervalValue ? 2 : 11 	// first time refresh in 7, after refresh in 2 seconds
	if (atomicState.backgroundActionInProgress != false) {
		return dynamicPage(name: "prefPkgRepairExecute", title: "", nextPage: "prefPkgRepairExecute", install: false, uninstall: false, refreshInterval: atomicState.refreshIntervalValue) {
			displayHeader(' Repair')
			section {
				showHideNextButton(false)
		...
		atomicState.refreshIntervalValue = null
...
def performUninstall() {
	...
	for (pkgToUninstall in pkgUninstall) {
		...
		for (bundle in pkg.bundles) {
			setBackgroundStatusMessage("Uninstalling ${bundle.name}")
			if (uninstallBundle(bundle.name)) {
				completedActions["bundleUninstalls"] << bundle
			}
			else {
...
def getRequiredAppsFromManifest(manifest) {
	def appsList = [:]
	for (app in manifest.apps) {
		if (app.required == true || app.required == "true")
			appsList << ["${app.id}":app]
	}
	return appsList
}
...
def getRequiredDriversFromManifest(manifest) {
	def driversList = [:]
	for (driver in manifest.drivers) {
		if (driver.required == true || driver.required == "true")
...
def getRequiredBundlesFromManifest(manifest) {
	def bundlesList = [:]
	for (bundle in manifest.bundles) {
		if (bundle.required == true || bundle.required == "true")
			bundlesList << ["${bundle.id}":bundle]
	}
	return bundlesList
}
...
def uninstallApp(id) {
	if (location.hub.firmwareVersionString >= "2.3.8.128") {
		try {
			def params = [
				uri: getBaseUrl(),
				path: "/app/edit/deleteJsonSafe/$id", 
				headers: [
					"Cookie": state.cookie
				],
				timeout: 300,
...
def rollback(error, runInBackground) {
	...
	if (installAction == "install" || installAction == "modify" || installAction == "update") {
		...
		for (installedBundle in completedActions["bundleInstalls"])
			uninstallBundle(installedBundle.name)
	}
